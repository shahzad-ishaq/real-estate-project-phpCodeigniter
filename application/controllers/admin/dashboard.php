...
//                         ')';
        ...
        $this->data['favories'] = $this->Favorites_m->get_joined(false, 5, 'property.id DESC', NULL);
        
		$where_in = array();
        
        foreach ($this->data['favories'] as $key => $value) {
           $where_in[]= $value->property_id;
        }
		
        $where['language_id']  = $this->data['content_language_id'];
        $this->data['fav_props'] = $this->estate_m->get_by($where,FALSE, NULL, NULL, NULL, array(), $where_in);
