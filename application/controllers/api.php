...
//}
  ...
  public function pdf_export($property_id = '', $lang_code = 'en') {
    ...
    public function login_form($lang_code = 'en') {
        ...
        if($this->form_validation->run() == TRUE)
        {
		//$userArray = $this->user_m->login();
		//echo '<pre>';
		//print_r($userArray);
		//exit('aad');
			$userArray = $this->user_m->login();
			$status = $userArray['status'];
			$userInfo = $userArray['userinfo'];
    ...
    public function register_form($lang_code = 'en') {
		...
		/*$data['name_surname'] = $this->input->get('name_surname');
		$data['username'] = $this->input->get('username');
	//	$data['mail'] = $this->input->get('mail');
		$data['password'] = $this->input->get('password');
		$data['password_confirm'] = $this->input->get('password_confirm');
                ...
                if (isset($tree_listings[$id]) && sw_count($tree_listings[$id]) > 0){
                    foreach ($tree_listings[$id] as $key => $_child) {
                        ...
                        if (isset($tree_listings[$_child->id]) && sw_count($tree_listings[$_child->id]) > 0){    
                            recursion_calc_sw_count($result_count, $tree_listings, $_parent_title, $_child->id, $ariesInfo);
                        }
                    }
                }
            };
            ...
            if(!empty($tree_listings)){
                ...
                foreach ($_treefields as $val) {
                    ...
                    if(isset($tree_listings[$val->id]) && sw_count($tree_listings[$val->id]) > 0){
                        ...
                        recursion_calc_sw_count($result_count, $tree_listings, $parent_title, $val->id, $ariesInfo);
                    }     
                }
                $count = array();
...
//                                                    array(), NULL, TRUE);
    ...
    public function getAllSlides()
	{
        ...
        // If not exists
        if(sw_count($this->data['slideshow']) == 0)
        {
            // Add one
    ...
    public function forgetpassword()
    {
	  ...
	  //  $this->user_m->loggedin() == FALSE || redirect($dashboard);
        ...
        if($this->form_validation->run() == TRUE)
        {
            ...
            $message.=site_url('api/resetpassword/'.$user->id.'/'.$new_hash)."\n\n";
            ...
            if ( ! $this->email->send())
            {
              //  $this->session->set_flashdata('error', 
               ...
               // redirect('api/forgetpassword', 'refresh');
				$msg = "Email sending problem, please contact administrator.";
				echo json_encode($msg); 
            }
            else
            {
				$msg = "Reset link sent to email, please check your email.";
                echo json_encode($msg); 
    ...
    public function resetpassword($user_id = NULL, $hash = NULL)
    {
        ...
        if($this->form_validation->run() == TRUE)
        {
            if($this->config->item('app_type') == 'demo')
            {
                $this->session->set_flashdata('error', 
                        lang('Data editing disabled in demo'));
                redirect('api/resetpassword/'.$user_id.'/'.$hash);
                exit();
            }
    ...
    function getAllAgents()
    {
        ...
        $this->load->model('user_m');
        $users = $this->user_m->get_Allagents();
    ...
    public function add_to_favorites($lang_code='')
    {
        ...
        if($this->favorites_m->check_if_exists($user_id, $property_id)>0)
        {
    ...
    function getAllMarkers()
    {
        ...
        $this->load->model('option_m');
        $allMarkers = $this->option_m->get_all_markers();
		
		$arr_vals = $allMarkers->values;
		$arr_list = 	explode(',',$arr_vals);
		
        if($arr_list)
        {
            $data['results'] = $arr_list;
        }
...
//                                                    array(), NULL, TRUE);
	...
	public function getagents()
    {
        ...
        $data['allagents'] = $this->user_m->get_Allagents();
        $this->data['allagents'] = array();
        $allagents = array();
        
        /*foreach ($this->data['allagents'] as $key => $value) {
           $allagents[]= $value->property_id;
        }
        */
        
        $all_agents = json_encode($data);
        echo $all_agents;
        exit;
    }
